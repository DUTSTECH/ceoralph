...
These files are provided for reference. Follow the patterns shown here.`);
    ...
    // Design context
    if (ctx.design) {
      const designParts: string[] = ["### Design Context"];
...
Implement this task now. Output the complete file contents for any files you create or modify using the FILE format. When done, output TASK_COMPLETE on its own line.`);
   ...
   */
  validate(ctx: ContextPackage): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (!ctx.task) {
      ...
      if (!ctx.task.title) errors.push("Missing task.title");
      if (!ctx.task.do) errors.push("Missing task.do");
      if (!ctx.task.doneWhen) errors.push("Missing task.doneWhen");
    }
